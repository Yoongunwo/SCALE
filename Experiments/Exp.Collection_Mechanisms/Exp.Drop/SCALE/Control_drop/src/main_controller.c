...
int init_drop_map(const char *path, __u32 pid) {
    ...
    __u64 zero = 0;
    if (bpf_map_update_elem(fd, &pid, &zero, BPF_ANY) != 0) {
        fprintf(stderr, "[-] Failed to init key %u in drop map: %s\n",
                pid, strerror(errno));
        return -1;
    }

    printf("[+] Initialized drop[%u] = 0\n", pid);
    return 0;
}
...
int main() {
    ...
    if (pin_map(skel->maps.drop, "/sys/fs/bpf/drop") !=0 ) {
        dispatcher_bpf__destroy(skel);
        return 1;
    }
    init_drop_map("/sys/fs/bpf/drop", 1);
