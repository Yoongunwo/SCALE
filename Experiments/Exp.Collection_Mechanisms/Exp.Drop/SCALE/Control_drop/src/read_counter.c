...
// extract a string field with jq
...
int main() {
    ...
    // drop map
    printf("\n[+] Dumping drop map:\n"); 
    int drop_fd = bpf_obj_get("/sys/fs/bpf/drop");
    if (drop_fd < 0) {
        perror("[-] Failed to open /sys/fs/bpf/drop");
        return 1;
    }

    int drop_count = 0;
    if (create_pid_to_podip_map(pairs, &drop_count) != 0) {
        fprintf(stderr, "[-] Failed to create PID↔IP map\n");
        return 1;
    }

    dump_counter_map(drop_fd, pairs, drop_count);
