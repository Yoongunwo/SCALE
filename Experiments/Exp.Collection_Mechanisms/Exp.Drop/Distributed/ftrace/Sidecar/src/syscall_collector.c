...
int main(void) {
    ...
    int host_tgid = find_host_tgid_by_cgroup_and_comm(HOST_PROC, cg, "postmark");
    if(host_tgid<=0){ fprintf(stderr,"[ERR] cannot map to host TGID\n"); return 1; }
    
    int h_tids[512]; 
    int n_htids = collect_host_tids(host_tgid, HOST_PROC, h_tids, 512);
